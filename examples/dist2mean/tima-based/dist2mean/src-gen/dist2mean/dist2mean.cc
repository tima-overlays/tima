			...
			tima::TimaNativeContext* ctx)
{
	...
	std::string tmp1 = "spontaneous awake";
	print_p(
		name, ctx,
		tmp1
	);
	std::string tmp2 = "This is a message";
	initial_dissemination_p(
		name, ctx,
		tmp2
	);
}
			...
			tima::TimaNativeContext* ctx)
{
	...
	std::string tmp1 = "Finally intermediary";
	print_p(
		name, ctx,
		tmp1
	);
...
MainPhase_stop_do(
			...
			tima::TimaNativeContext* ctx)
{
	std::string tmp1 = "Stop broadcasting";
	print_p(
		name, ctx,
		tmp1
	);
